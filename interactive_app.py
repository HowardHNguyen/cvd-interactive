...
# App config (just displayed, not required for routing)
APP_URL = "https://cvd-predictor.streamlit.app/"
TARGET_FHX_OR = 2.5   # epidemiology-aligned OR for family history
SMOOTH_EPS = 1e-6     # clamp to avoid 0/1 edge cases
...
@st.cache_resource
def load_data():
    """Load dataset from repo root (expects one of: data_cvd_perfect_300.csv, data_ehr_500.csv)."""
    for path in ["data_cvd_perfect_300.csv", "data_ehr_500.csv"]:
        if os.path.exists(path):
            try:
                ...
                pass
    st.error("No valid dataset found in app root. Place data_cvd_perfect_300.csv or data_ehr_500.csv in the repo root.")
    st.stop()
